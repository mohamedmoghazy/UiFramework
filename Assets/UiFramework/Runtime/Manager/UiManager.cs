...
namespace UiFramework.Runtime.Manager
{
    public class UiManager : MonoBehaviour
    {
        ...
        private async Task LoadAndPushState(UiStateEntry entry, object context, bool additive)
        {
            ...
            {
                // Start loading the new state's scenes before unloading previous states.
                // This avoids races where unloading addressables removes assets needed for the new state.
                newState.Init(context);

                // Wait for the new state's scene loads to finish so we can pass their scene names
                // to the unload step (so they are preserved).
                await newState.WaitForInitializationAsync();

                List<string> keepScenes = newState.GetUiElementSceneNames();

                // Now unload previous states but keep scenes that belong to the new state.
                await UnloadAllPreviousStates(keepScenes);
                stateStack.Clear();

                // Push the already-loaded new state onto the stack. Do NOT call Init again.
                stateStack.Push(newState);
                return;
            }

            // Additive or no previous states: behave as before (push then init)
            stateStack.Push(newState);
            newState.Init(context);
        }
